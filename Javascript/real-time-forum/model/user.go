...
func (u *User) Create(db *sql.DB) error {
	result, err := db.Exec("INSERT INTO users(uuid,username, firstname, lastname, sex, birthday, email,password) values(?,?,?,?,?,?,?,?)", u.UUID, u.Username, u.Firstname, u.Lastname, u.Gender, u.Age, u.Email, u.Password)
	if err != nil {
...
func (u *User) UpdateUser(db *sql.DB) error {
	...
	&user.Id, &user.UUID, &user.Username, &user.Firstname, &user.Lastname, &user.Gender, &user.Age, &user.Email, &user.Password
		uuid = ?,
		username = ?,
		firstname = ?,
		lastname = ?,
		...
		password = ?,
		WHERE id = ?`,
		u.UUID, u.Username, u.Firstname, u.Lastname, u.Gender, u.Age, u.Email, u.Password, u.Id); err != nil {
		return errors.New(logger.GetCurrentFuncName() + " " + err.Error())
	}
	return nil
}
...
func (u *User) GetPosts(db *sql.DB) ([]*Post, error) {
	return GetPostsByUserId(db, u.Id)
}
...
func GetUsers(db *sql.DB, exclude ...int64) ([]*User, error) {
	...
	for rows.Next() {
		var user User
		err = rows.Scan(&user.Id, &user.UUID, &user.Username, &user.Firstname, &user.Lastname, &user.Gender, &user.Age, &user.Email, &user.Password)
		if err != nil {
			return nil, errors.New(logger.GetCurrentFuncName() + " " + err.Error())
		}
...
func GetUserById(db *sql.DB, Id int64) (*User, error) {
	...
	user := &User{}
	if err := row.Scan(&user.Id, &user.UUID, &user.Username, &user.Firstname, &user.Lastname, &user.Gender, &user.Age, &user.Email, &user.Password); err != nil {
...
// Creates a new user
func CreateUser(db *sql.DB, username string, firstname string, lastname string, gender string, age string, email string, password string) (*User, error) {
	user := &User{
		Username:  username,
		Firstname: firstname,
		Lastname:  lastname,
